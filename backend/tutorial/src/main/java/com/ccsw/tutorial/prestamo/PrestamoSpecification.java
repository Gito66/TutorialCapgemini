...
public class PrestamoSpecification implements Specification<Prestamo> {
    ...
    public Predicate toPredicate(Root<Prestamo> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        ...
        if (criteria.getOperation().equalsIgnoreCase(">=") && criteria.getValue() != null) {
            ...
            if (path.getJavaType() == String.class) {
                return builder.greaterThanOrEqualTo((Path<String>) path, "%" + (String) criteria.getValue() + "%");
            } else {
                // return builder.greaterThanOrEqualTo((Path<LocalDate>) path, (LocalDate)
                // criteria.getValue());
        ...
        if (criteria.getOperation().equalsIgnoreCase("<=") && criteria.getValue() != null) {
            Path<?> path = getPath(root);
            if (path.getJavaType() == String.class) {
                return builder.lessThanOrEqualTo((Path<String>) path, "%" + (String) criteria.getValue() + "%");
            } else {
